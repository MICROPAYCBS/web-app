...
| CL-038a | Guided savings deposit/withdrawal import | Org bulk-import Savings Transactions | `/savings/import` analyze → post; org bulk-import remains legacy | Done |
| CL-039 | Fixed deposits        | `/fixed-deposits`                     | General → FD                  | Partial (list + create + detail panels + lifecycle, modify, add charge, txn actions)  |
| CL-040 | Recurring deposits    | `/recurring-deposits`                 | General → RD                  | Partial (list + create + detail panels + lifecycle, cashier deposit/withdrawal, modify, add charge, txn actions)  |
