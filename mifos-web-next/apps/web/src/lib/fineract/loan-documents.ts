...
export async function getLoanDocuments(
  ...
  const data = await fineract.get<FineractEntityDocument[]>(`/loans/${accountId}/documents`);
...
export async function uploadLoanDocument(
  ...
  const res = await fetch(fineractUrl(urlBase, `/loans/${accountId}/documents`), {
    ...init,
    headers,
    body: formData
  });
...
export async function deleteLoanDocument(
  ...
  return fineract.delete<FineractCommandProcessingResult>(
    `/loans/${accountId}/documents/${documentId}`
  );
}

export async function fetchLoanDocumentAttachment(
  accountId: string | number,
  documentId: number
): Promise<Response> {
  const { urlBase, init } = await buildFineractRequestInit();
  return fetch(
    fineractUrl(urlBase, `/loans/${accountId}/documents/${documentId}/attachment`),
    init
  );
}
