...
export default async function EditFixedDepositProductPage({
  params
}: {
  ...
  try {
    assertCan(session, resolvePermission('products.fixedDeposit.update'));
  } catch {
    redirect('/forbidden');
  }
  ...
  let rawTemplate;
  try {
    rawTemplate = await getDepositProductForEdit('fixed', productId);
  } catch (err) {
