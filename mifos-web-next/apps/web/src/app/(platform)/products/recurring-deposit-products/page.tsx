...
export default async function RecurringDepositProductsPage() {
  const session = await getServerSession();
  if (!can(session, resolvePermission('products.recurringDeposit'))) {
    notFound();
  }

  const products = await listDepositProducts('recurring');

  return <DepositProductsPageContent kind="recurring" products={products} />;
}
