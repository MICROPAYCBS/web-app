...
import { fetchLoanDocumentAttachment } from '@/lib/fineract/loan-documents';
...
export async function GET(
  ...
  try {
    assertCan(session, resolvePermission('loans.documents'));
    const { accountId, documentId } = await context.params;
    const disposition = parseDocumentDispositionParam(
      new URL(request.url).searchParams.get('disposition')
    );
    const res = await fetchLoanDocumentAttachment(accountId, Number(documentId));
    if (!res.ok) {
