...
import type {
  ...
  ChartOfAccountsImportRowProgress
} from '@/lib/accounting/chart-of-accounts-import';
import { cn } from '@/lib/utils';

function progressLabel(progress: ChartOfAccountsImportRowProgress | undefined) {
  if (!progress) {
    return 'Ready';
  }
...
export function ChartOfAccountsImportReview({
  analysis,
  progressByRow,
  importing = false
}: {
  analysis: ChartOfAccountsImportAnalysis;
  progressByRow?: Record<number, ChartOfAccountsImportRowProgress>;
  importing?: boolean;
}) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            {analysis.rowCount} account{analysis.rowCount === 1 ? '' : 's'} found.
            {showProgress
              ? ` ${completedCount} created${failedCount > 0 ? `, ${failedCount} failed` : ''}.`
              : analysis.errorRowCount > 0
                ? ` ${analysis.errorRowCount} row${analysis.errorRowCount === 1 ? '' : 's'} need attention before creating accounts.`
        ...
        <table className="w-full min-w-[960px] text-left text-sm">
          <thead className="border-b border-border bg-muted/40 text-muted-foreground">
            <tr>
              ...
              <th className="px-3 py-2 font-medium">Manual entries</th>
            ...
            {analysis.rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2 align-top">{row.accountName || '—'}</td>
                  <td className="px-3 py-2 align-top">{row.usage || '—'}</td>
                  <td className="px-3 py-2 align-top">{row.manualEntriesAllowed || '—'}</td>
                  <td className="px-3 py-2 align-top font-mono text-xs">{row.glCode || '—'}</td>
                  <td className="px-3 py-2 align-top">
                    {row.parentId ? (
                      <span>
                        {row.parent || '—'}{' '}
                        <span className="text-muted-foreground">(#{row.parentId})</span>
                      </span>
                    ) : (
                      row.parent || '—'
                    )}
