...
import {
  createLoanNoteAction,
  deleteLoanNoteAction,
  updateLoanNoteAction
} from '@/actions/loan-note';
import type { LoanAccountNotesContext } from '@/components/clients/loan-account/loan-account-related-context';
...
export function LoanAccountNotesSection({
  clientId,
  accountId,
  context
}: {
  clientId: string;
  accountId: number;
  context: LoanAccountNotesContext;
}) {
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      const result = await createLoanNoteAction(clientId, accountId, { note: draft.trim() });
      if (!result.ok) {
  ...
  function handleEditSave() {
    ...
    setActionError(null);
    startTransition(async () => {
      const result = await updateLoanNoteAction(clientId, accountId, editTarget.id, {
        note: editDraft.trim()
      });
      if (!result.ok) {
        setActionError(formatActionErrorMessage(result.message, result.fieldErrors));
  ...
  function handleDeleteConfirm() {
    ...
    setActionError(null);
    startTransition(async () => {
      const result = await deleteLoanNoteAction(clientId, accountId, deleteTarget.id);
      if (!result.ok) {
        setActionError(formatActionErrorMessage(result.message, result.fieldErrors));
        return;
      }
  ...
  return (
    <DetailSection title="Notes">
      <p className="text-sm text-muted-foreground">Internal notes about this loan account.</p>

      {context.canCreate ? (
        <div className="mt-4 space-y-2">
          <Label htmlFor="loan-note-draft">Add a note</Label>
          <Textarea
            id="loan-note-draft"
            value={draft}
      ...
      {context.items.length === 0 ? (
        <div className="mt-4">
          <EmptyState
            icon={NotebookPen}
            title="No notes yet"
            description="Notes help your team track conversations, follow-ups, and context for this loan."
          />
        </div>
      ) : (
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Update the note text for this loan.</DialogDescription>
          </DialogHeader>
          <Textarea
            value={editDraft}
          ...
          <DialogFooter>
            ...
            <Button type="button" disabled={pending || !editDraft.trim()} onClick={handleEditSave}>
              Save
            </Button>
          </DialogFooter>
        </DialogContent>
