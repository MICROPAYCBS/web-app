...
import type { SavingsAccountsPage, SavingsAccountListItem } from '@mifos/api-client';
import {
  getCoreRowModel,
  useReactTable,
  type ColumnDef,
...
import {
  ...
  savingsListSortColumnFromField,
  savingsListSortField,
...
export function SavingsAccountsTable({
  ...
  const [sortColumn, setSortColumn] = useState<PortfolioListSortColumn | undefined>(
    initialQuery.orderBy ? savingsListSortColumnFromField(initialQuery.orderBy) : undefined
  );
  ...
  const syncUrl = useCallback(
    (query: PortfolioListQuery) => {
      ...
      router.replace(qs ? `/savings?${qs}` : '/savings', { scroll: false });
    },
    [router]
  );
    ...
    (query: PortfolioListQuery) => {
      startTransition(async () => {
        const res = await fetch(
          `/api/savingsaccounts?${new URLSearchParams(buildPortfolioListApiQuery(query)).toString()}`,
          { credentials: 'include' }
        );
        if (!res.ok) {
          return;
        }
        const json = (await res.json()) as SavingsAccountsPage;
        setData(json);
        syncUrl(query);
      });
    },
  ...
  useEffect(() => {
    ...
    fetchPage({
      ...
      includeClosed,
      orderBy: sortColumn ? savingsListSortField(sortColumn) : undefined,
      sortOrder
    });
  ...
  const columns = useMemo<ColumnDef<SavingsAccountListItem>[]>(
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Link
              href={clientAccountGeneralPath(clientId, 'savings', id)}
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <Checkbox
            id="savings-include-closed"
            checked={includeClosed}
            onCheckedChange={(checked) => setIncludeClosed(checked === true)}
          />
          <Label htmlFor="savings-include-closed" className="font-normal">
