...
import { formatGenderLabel } from '@/lib/fineract/client-title-eligibility';
...
function genderLabel(genderId: number | null | undefined, template: CustomerTitleTemplate): string {
  ...
  return (
    template.genderOptions.find((option) => option.id === genderId)?.name ??
    formatGenderLabel(genderId) ??
    String(genderId)
  );
}
