...
function chargeOptionsForIds(
  options: LoanProductStepProps['template']['chargeOptions'],
  selectedIds: number[],
  currencyCode?: string
) {
...
export function MappingsStep({
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedChargeIds.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          No fees or penalties are selected on the Charges step. You can still map any available
          charge, or add charges first and return here.
        </p>
      ) : null}
    </div>
  );
